...
open class ARBathymetryNode: LocationNode {
  ...
  init(
    ...
    let offset = (bathymetryTile.ne.latitude - bathymetryTile.sw.latitude)
    super.init(location: CLLocation(coordinate: CLLocationCoordinate2D(latitude: bathymetryTile.sw.latitude + offset, longitude: bathymetryTile.sw.longitude), altitude: altitude))
